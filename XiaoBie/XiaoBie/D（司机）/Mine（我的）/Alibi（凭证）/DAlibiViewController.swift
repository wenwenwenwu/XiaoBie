...
class DAlibiViewController: UIViewController, UITableViewDataSource, UITableViewDelegate {
    ...
    //MARK: - Properties
    lazy var photoButtonView1 = DPhotoButtonView.viewWith(uploadPara: "upload_daily_evidence", ownVC: self) { [weak self] in
        self?.photoButtonView2.isHidden = false
    }

    lazy var photoButtonView2 = DPhotoButtonView.viewWith(uploadPara: "upload_daily_evidence", ownVC: self) { [weak self] in
        self?.photoButtonView3.isHidden = false
    }

    lazy var photoButtonView3 = DPhotoButtonView.viewWith(uploadPara: "upload_daily_evidence", ownVC: self) {
