    ...
    if (data)
    {
        ...
        if ([dict isKindOfClass:[NSDictionary class]])
        {
            if ([dict jsonInteger:NTESNotifyID] == NTESCustom)
            {
                ...
                //fixed by dd tabbar
                UIView *navView = MainTabbarController.instance.selectedViewController.view;
                [navView makeToast:content duration:2.0 position:CSToastPositionCenter];
...
//    UINavigationController *nav = tabVC.selectedViewController;
    ...
    if ([self shouldResponseBusy]){
        ...
        if ([NSThread isMainThread])
        {
            ...
            if ([nav.visibleViewController isKindOfClass:[DDChatSessionViewController class]]) {
                //当前就是聊天界面
            }else{
                [DDChatManager pushToSessionWithServiceIdWithUserId:caller nav:nav serviceId:extendMessage];
            }
...
//    [tabVC.view endEditing:YES];
    ...
    MainTabbarController *tabVC = MainTabbarController.instance;
    [tabVC.view endEditing:YES];
    tabVC.tabBarController.tabBar.hidden = YES;
