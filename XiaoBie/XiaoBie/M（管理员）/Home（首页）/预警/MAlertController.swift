...
class MAlertController: UIViewController, UITableViewDataSource, UITableViewDelegate {
    ...
    //MARK: - Request
    func loadRequest() {
        WebTool.post(uri:"customer_server_alarm", para:["staff_id":AccountTool.userInfo().id, "page_num":"1", "page_size":pageSize], success: { (dict) in
            let model = MAlertListResponseModel.parse(dict: dict)
            if model.code == "0" {
    ...
    func loadMoreRequest() {
        WebTool.post(uri:"customer_server_alarm", para:["staff_id":AccountTool.userInfo().id, "page_num":String(pageCount), "page_size":pageSize], success: { (dict) in
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        let cell = MAlertCell.cellWith(tableView: tableView)
